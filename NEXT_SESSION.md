...
### 残る仮説（要次回検証）
**「DTB ロードが PCIe クロック起動のトリガー」**:
- u-boot の通常 boot 手順では config.txt に `device_tree=bcm2711-rpi-4-b.dtb` が指定され、firmware が
  DTB を読み込みデバイス init を実行
- 我々の config.txt には **`device_tree=` 指定なし** → firmware は default DTB を探すが、ロード成功
  しても処理が違うかも
- **次回試行**: config.txt に明示的に `device_tree=bcm2711-rpi-4-b.dtb` を追加、SD root に DTB が
  あるか確認、無ければ raspberrypi/firmware から fetch
