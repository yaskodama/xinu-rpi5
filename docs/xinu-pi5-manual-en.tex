...
\cmd{wifi on/off/status/scan} & WiFi operations (Section~\ref{sec:wifi}) \\
...
Credentials are embedded into the kernel at build time, and the system
\textbf{does not auto-connect at boot}. It connects only when \cmd{wifi on}
is run.
...
A WiFi signal icon is drawn at the bottom-right of the screen, with the
SSID and local IP shown beneath it.
