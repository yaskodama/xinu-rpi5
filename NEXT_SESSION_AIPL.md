...
| 焼いてある | **`3df7543f…`**（2,163,096 B） | **`1fb2852a…`**（2,045,080 B） |
...
| 正典ガイド 10 本 | **8 本が完全一致**（実機・出力照合） | **10 本すべて一致**（`/cc` だけで） |

**2026-09-04 に両機の機能を揃えた。** 残る差は 2 つだけで、どちらも実行の器の性質:
...
1. 真偽値を `true` / `false` で印字する（両機とも `1`）。値にタグが要るので
   Pi 3 は VM 変更＝焼き直し、Pi 5 はランタイム変更。**両機同時に入れること**
   （片方だけ入れると、せっかく揃えた表示がまた食い違う）。
