...
| 焼いてある | **`820388da…`**（2,163,160 B） | **`fc71e715…`**（2,045,080 B） |
...
| 正典ガイド 10 本 | **9 本が完全一致**（実機・出力照合） | **10 本すべて一致**（`/cc` だけで） |

**2026-09-04 に両機の機能を揃えた。** 残る差は **g9 の順序だけ**:

| | 中身 |
|---|---|
| g9 の順序 | Pi 3 は `got actor / pong / after send` になることがある（実プロセスなので send の相手が先に走る。回による）。Pi 5 は必ず正典どおり。**どちらも AIPL としては正しい**（send は順序を約束しない） |

**g8 の `b = true` も同日中に揃えた。** 真偽値をタグ付きの値にした
（Pi3 = `0x20000000`/bit29、Pi5 = bit62・最下位ビット 0）。
**Pi 5 で整数側にタグは載せられない** —— `v_int(-1)` は上位ビットが全部立つので、
上位ビットを印にすると負の数が真偽値に化ける。
**★ 文字列と違い、真偽値は条件判定でもタグを見る**（`false` はタグのぶん 0 ではないので、
素の JZ では偽にならない）。Pi3 = `vm_falsy`、Pi5 = `v_truthy` が先に見る。
**Mac 側 `--xinu-jit`（`POST /actor/load`）だけは `1` のまま** —— 前段が
`true`/`false` を `Int 1`/`Int 0` に潰すため。直すと型推論まで波及する。

**g7（`wait`）は同日中に解消した。** `/cc` を「専用プロセス・静的スタックで走らせて
消えるまで `proc_resched()` を回す」形（＝シェルの `cc` と同じ）にしたので、
`/cc` だけで `fast = 42 / slow(timed out) = 0 / await = 10`（0.40 秒）。

### ★ ここに板を半分殺す罠がある（実機で踏んだ）

**ランナーを進めているのは、呼び出し元が `proc_resched()` を回していることだけ。**
`proc_preempt()` は `currpid == NULLPROC` のとき切り替えないので、
見切って置き去りにしたランナーは**二度と走らない**。静的スタックと `g_ccrun_pid` を
握ったまま残り、以後 `/cc` もシェルの `cc` も「前の実行がまだ終わっていません」しか
返さなくなる（要再起動）。三重に塞いだ:
1. 眠れる合計を 1 回 10 秒（見切りの 15 秒を確実に跨がない）
2. 見切るとき `cc_request_abort()` で「起きたら即抜けろ」
3. 次の呼び出しが 3 秒だけ回して片づける

**「その場で眠る」は採ってはいけない。** HTTP の処理は `net_proc_main` からも
wm ループ（NULLPROC）からも入りうる（`loader/main.c` の `net_proc_main` と
`serial_io_tick` が両方 `genet_rx_tick` を呼び、`g_rx_busy` で片方が空振りする）。
NULLPROC で `proc_sleep_us` すると ready が空のとき自分自身へ ctxsw して眠らない。
※ `cc.c` のコメント「JIT は ISR の中で走る」は RT-port Stage 2 の時点で古い。
...
| **`/cc` でも `wait()`**（専用プロセス化） | `system/tcp_server.c`, `shell/shell.c`, `cc/cc.c`（`6e83930`） | g7 が `/cc` だけで正典どおり＝**経路の使い分けが不要に** |
| **真偽値のタグ**（`b = true`） | `cc/cc.c`, `cc/abcl2c.c`（`bb61da8`）＋ `aice-avm`（`9c6676d`）＋ `xinu-rpi3`（`115e30e`） | 両機同時に。**片方だけ入れると表示がまた食い違う** |
...
1. Mac 側 `--xinu-jit`（`POST /actor/load`）の真偽値。前段（`src/parser.mly:327`）が
   `TRUE -> Int 1` に潰している。構文木に真偽値を足す＝`infer.ml` と
   `c_translator.ml` まで波及するので、機内経路（`/cc`）で足りるうちは後回しでよい。
2. Pi 3 の `ai_call` が遅くなる件（未解明。起動直後 2--6 秒 → 4.4 秒を実測。
   常駐アクタが増えるほど遅い、が有力だが切り分けていない）
3. `/cc` の実行中は RX を汲まない（呼び出し元が `proc_resched()` を回している間、
   `g_rx_busy` が立ったまま）。`wait` が長いプログラムでは HTTP が数秒止まる。
   気になるなら、応答を後から返す形（202 + 取りに行く口）へ。
