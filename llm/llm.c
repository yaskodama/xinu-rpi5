...
static double l_exp(double x)
{
    ...
    /* AArch64 では unsigned long は 64 ビットだが、ARM32 では 32 ビットしかない。
       Pi3 への移植でここが原因で exp が壊れ、<unk> だけを出し続けた。
       どの環境でも 64 ビットの unsigned long long を使い、幅はビルドで検査する。 */
    union { unsigned long long u; double d; } sc;
    _Static_assert(sizeof(sc.u) == sizeof(sc.d), "l_exp: 共用体の幅が double と違う");
    sc.u = (unsigned long long)(k + 1023) << 52;     /* 2^k */
    return p * sc.d;
}
